            ...
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex space-x-2">
                        <a href="{{ route('transaksi.create-barang-masuk') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                            Tambah Barang Masuk
                        </a>
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                    <form method="GET" action="{{ route('transaksi.barang-masuk') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                    ...
                    <table class="min-w-full bg-white border border-gray-200">
                        <thead class="bg-green-50">
                            <tr>
                                ...
                                <th class="py-2 px-4 border-b text-left text-sm font-medium text-gray-900">Supplier</th>
                        ...
                        <tbody>
                            @forelse($transaksis as $transaksi)
                                <tr class="hover:bg-gray-50">
                                    <td class="py-2 px-4 border-b text-sm font-mono">{{ $transaksi->kode_transaksi }}</td>
                                    <td class="py-2 px-4 border-b text-sm">{{ $transaksi->formatted_tanggal }}</td>
                                    <td class="py-2 px-4 border-b text-sm">
                                        <div class="font-medium">{{ $transaksi->barang->nama_barang }}</div>
                                        <div class="text-xs text-gray-500">{{ $transaksi->barang->kode_barang }}</div>
                                    </td>
                                    <td class="py-2 px-4 border-b text-sm">
                                        <span class="font-medium text-green-600">+{{ $transaksi->jumlah }}</span> 
                                    ...
                                    <td class="py-2 px-4 border-b text-sm">{{ $transaksi->supplier ?? '-' }}</td>
                                    ...
                                    <td class="py-2 px-4 border-b text-sm">
                                        <div class="flex space-x-2">
                                            ...
                                            <form action="{{ route('transaksi.destroy', $transaksi) }}" method="POST" class="inline" 
                                                  onsubmit="return confirm('Yakin ingin menghapus transaksi ini? Stok akan dikurangi.')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="9" class="py-8 px-4 text-center text-gray-500">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <a href="{{ route('transaksi.create-barang-masuk') }}" class="mt-2 text-green-600 hover:text-green-900">
